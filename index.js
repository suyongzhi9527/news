...
let direction = 'after';
let show_only = false;
let DataHost = 'http://127.0.0.1:62/'
...
function first_xhr() {
    ...
    try {
        ...
        xhr_zhihu.open('GET', DataHost + 'api?origin=zhihu&_vercel_no_cache=1' + '&cache=' + now_time);
    ...
    try {
        ...
        xhr_163.open('GET', DataHost + 'api?origin=163&_vercel_no_cache=1' + '&cache=' + now_time);
...
function get_day_news(index, origin) {
    try {
        ...
        xhr.open('GET', DataHost + `api?index=${index}&cache=${cache}&origin=${origin}`);
